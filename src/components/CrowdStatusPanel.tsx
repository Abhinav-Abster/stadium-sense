...
export default function CrowdStatusPanel() {
  ...
  const [stadiumId, setStadiumId] = useState('metlife-stadium');

  const { data, loading, error, execute } = usePostJson<CrowdData>(
    '/api/crowd-status',
    `${t('loading')} error. Please retry.`
  );

  const fetchStatus = useCallback(
    (targetId: string) => execute({ stadiumId: targetId }),
    [execute]
  );

  // Initial fetch and auto-refresh
  useEffect(() => {
    const timer = setTimeout(() => fetchStatus(stadiumId), 0);
    const interval = setInterval(() => fetchStatus(stadiumId), 30000);
    return () => {
      clearTimeout(timer);
  ...
  return (
    ...
    >
      ...
      {error && (
        <div
          role="alert"
          className="p-4 rounded-lg bg-red-900/50 border border-red-700/50 text-red-300 mb-4 text-sm"
        >
          {error}
        </div>
      )}
          ...
          >
            ...
            <div className="space-y-2 flex-1">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span
                  className={`px-2 py-0.5 text-xs font-semibold rounded-full border ${alertConfig?.badgeBg}`}
                >
                  {alertConfig?.label}
                </span>
